...
export function LogoutButton({ className = '' }) {
    ...
    return (
        <button
            className={`${styles['button--logout']} ${className}`}
            onClick={logout}
        >
            <Icons.Logout />
            Logout
...
export const Button = ({
    ...
    return modal ? (
        <ModalProvider onSubmit={onClick}>
            <ButtonWithModal text={text} {...buttonProps} />
            {modal}
        </ModalProvider>
    ) : (
        <button
...
function ButtonWithModal({ children = null, text, ...buttonProps }) {
    const { open } = useContext(ModalContext);

    return (
        <>
            <Button onClick={open} text={text} {...buttonProps} />
            {children}
        </>
    );
}
