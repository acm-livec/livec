    ...
    return (
        <div className={styles['suggestion-box']}>
            ...
            {response && response.success === true && (
                <Success onClose={close} response={response} />
            )}
            {response && response.success === false && <Failure />}
            {user && !response && (
                <Default sectionId={sectionId} submit={submit} />
            )}
        </div>
    );
}
...
const Default = ({ sectionId, submit }) => {
    ...
    return (
        <>
            <h2>
                Have a thought or suggestion for this section? Fill out the form
                below.
            </h2>
            <Form
                resetOn={[sectionId]}
                onSubmit={(formData) =>
                    submit({ sectionId, discipline, ...formData })
                }
            >
                <Dropdown
                    keyName="type"
