...
export function FontSizeToolbarButton() {
  ...
  return (
    <div className="flex w-fit h-7 items-center gap-1 rounded-md bg-muted/60 p-0">
      ...
      <Popover open={isFocused} modal={false}>
        <PopoverTrigger asChild>
          <input
            className={cn(
              'h-fit w-0 shrink-0 bg-transparent px-1 text-center text-sm hover:bg-muted'
            )}
            value={displayValue}
            onBlur={() => {
              setIsFocused(false);
        ...
        <PopoverContent className="w-fit px-px py-1" onOpenAutoFocus={(e) => e.preventDefault()}>
          {FONT_SIZES.map((size) => (
            <button
              key={size}
              className={cn(
                'flex h-8 w-fit items-center justify-center text-sm hover:bg-accent data-[highlighted=true]:bg-accent'
              )}
              onClick={() => {
