...
const toggleBlock = (editor, format) => {
    ...
    Transforms.unwrapNodes(editor, {
        match: (n) =>
            LIST_TYPES.includes(
                !SlateEditor.isEditor(n) && SlateElement.isElement(n) && n.type
            ),
        split: true,
    });
...
const jsonToSlateNodes = (blocks) => {
    const nodeForBlock = (block) => {
        if (block.items) {
            ...
            if (!htmlList) {
                htmlList = CUSTOM_ORDERED_LIST_TYPES.includes(
            ...
            const listType =
                htmlList === 'ol' ? 'numbered-list' : 'bulleted-list';
    ...
    return (
        ...
        >
            ...
            <div contentEditable={false} style={{ marginBottom: '0.5rem' }}>
                ...
                <BlockButton
                    action={(ed) => toggleBlock(ed, 'heading')}
                    icon="H2"
                />
